                                              ...
                                              Eigen::Matrix4f& pose_estimited)
{
    ...
    // {
    //     if (pre_init){
    //         ResetLocalFeatureMap(0, 0, 0);
    //         pose_estimited = Eigen::Matrix4f::Identity();
    //     }else{
    //         sc_relocation(cloud_data, init_pose_guess);
    //         std::cout << "init_pose_guess" << init_pose_guess << std::endl;
    //         ResetLocalFeatureMap(init_pose_guess(0,3), init_pose_guess(1,3), 0);
    //         pose_estimited = init_pose_guess;
    //     }
    //     /* ResetLocalFeatureMap(0, 0, 0);
    //     //ResetLocalFeatureMap(init_pose_guess(0,3), init_pose_guess(1,3), init_pose_guess(2,3));
    //     pose_estimited = Eigen::Matrix4f::Identity();
    //     //pose_estimited = init_pose_guess; */
        
    //     //auto detectResult = loam_sc_ptr_->detectLoopClosureID();
    //     /* loam_sc_ptr_->makeNewScancontextAndKeys(*cloud_data.cloud_ptr);
    //     std::vector<size_t> possibe_pose_candidates;
    //     loam_sc_ptr_->detectPossibleInitAera(possibe_pose_candidates);
    //     for (size_t i = 0; i < possibe_pose_candidates.size(); ++i){
    //         std::cout << "index:" << possibe_pose_candidates[i] << std::endl;
    //     }
    //     std::ifstream fin_ring_key(possible_pose_path_);
    //     if(!fin_ring_key){
    //         std::cout << "Cannot load fin_ring_key file at : " << possible_pose_path_ << std::endl;
    //     }
    //     //std::vector<Eigen::Matrix4f> possible_pose_;
    //     //candidate_aera.clear();
    //     ReadPossibleInitPose(possibe_pose_candidates, fin_ring_key, candidate_aera); */
    ...
    //std::cout << "pose_estimited" << pose_estimited << std::endl;
